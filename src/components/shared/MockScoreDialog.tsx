 ...
 */
export function MockScoreDialog({ open, onClose, onConfirm }: MockScoreDialogProps) {
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-5 py-5">
          <p className="text-sm text-muted-foreground">
            Informe o percentual de acerto obtido no simulado de hoje.
          </p>
...
function ScorePreview({ score }: { score: number }) {
  ...
  const level =
    clamped >= 85
      ? { label: "Excelente!", color: "bg-emerald-500/15 text-emerald-600 dark:text-emerald-400" }
      : clamped >= 75
        ? { label: "Bom resultado", color: "bg-amber-500/15 text-amber-600 dark:text-amber-400" }
        : { label: "Precisa melhorar", color: "bg-red-500/15 text-red-600 dark:text-red-400" };

  return (
    <div className={cn("rounded-md px-3 py-2 text-sm font-medium", level.color)}>
      {clamped}% — {level.label}
    </div>
  );
}
